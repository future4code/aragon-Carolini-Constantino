...
export class UserController {
    ...
    public login = async (req: Request, res: Response) => {
        ...
        try {
            ...
            const userDatabase = new UserDatabase()
            const userDB = await userDatabase.findByEmail(email)//retorna dados do usuário q contém o mesmo email passado via body

            if (!userDB) {
                errorCode = 401
                throw new Error("Email não cadastrado")
            }
            //"desfragmenta" as infos do usuário q retornou na const userDB.
            const user = new User(
                userDB.id,
                userDB.nickname,
                userDB.email,
            ...
            //método que compara se a senha recebido, depois de hasheada é a mesma salva no banco de dados:
            ...
            //como não é possível desfragmentar só partes de uma entidade, é possível chamar métodos que resultam nos atributos desejados (getId())
            const payload: ITokenPayload = {
                id: user.getId(),
                role: user.getRole()
            }
    ...
    public editUser = async (req: Request, res: Response) => {
        ...
        try {
            ...
            if (email && (!email.includes("@") || !email.includes(".com"))) {
                throw new Error("O parâmetro email deve conter o '@' e o .com")
            }

            const userDatabase = new UserDatabase()
            const isUserExists = await userDatabase.checkIfExistsById(payload.id)
            const isIdExists = await userDatabase.checkIfExistsById(id)
